...
while True:
    ...
    if pergunta == 0:
        ...
        if estoque_consult == 9:
            ...
            encontrado = False
            for produ in produto:
                if codigo == produ['codigo_do_produto']:
                    encontrado = True
                    print("Este é o produto encontrado:")
                    print(f"Nome: {produ['nome']} -- Código: {produ['codigo_do_produto']} -- "
                          f"Quantidade: {produ['quantidade']} -- Preço unitário: R${produ['preço_unitario']:.2f}")
